...
func initConfig() {
	...
	for _, name := range []string{"app.conf", "config.yaml", "reply.php"} {
		...
		if len(s) == 0 {
			...
			r, err := httplib.Get(GhProxy + "https://raw.githubusercontent.com/SamDudu/xdd/main/conf/demo_" + name).Response()
			if err == nil {
				io.Copy(f, r.Body)
			}
		}
	...
	if ExecPath == "/Users/SamDudu/Desktop/xdd" || Config.NoAdmin {
		Cdle = true
	}
	if Config.Master == "" {
		Config.Master = "xxxx"
